...
Директорія [scripts/build](/scripts/build) містить скрипти оболонки, які виконують кроки конфігурації, побудови і встановлення проекту системою CMake, отож цілий цикл `Release` або `Debug` виконується однією командою замість окремих викликів CMake, описаних у секції [Побудова проекту через командний рядок](/doc/sections/uk_UA/5-project-build/5-2-command-line-build.md).
...
Цей проект не конфігурується без бібліотеки [ImagesAnnotator-DataDrivers](https://github.com/yuriysydor1991/ImagesAnnotator-DataDrivers.git), яку він знаходить викликом `find_package()` і ніколи не завантажує (див. секцію [Залежність від бібліотеки драйверів даних](/doc/sections/uk_UA/5-project-build/5-36-the-data-drivers-dependency.md)), отож ту бібліотеку слід встановити один раз перед запуском будь-якого зі скриптів нижче.

Цей проект також є бібліотекою, отож його побудова не створює нічого, що можна запустити: кроком, який робить результат придатним для використання, є встановлення, яке публікує пакунок `find_package()`, споживаний у секції [Використання бібліотеки у власному проекті](/doc/sections/uk_UA/8-using-the-library-in-your-project/8-using-the-library-in-your-project.md). Цей крок виконується лише тоді, коли вказано параметр `--install` із секції [Параметри, які приймають скрипти](#параметри-які-приймають-скрипти) нижче.
...
Встановлення драйверів даних поза типовими префіксами пошуку CMake вказується так само, бо `CMAKE_PREFIX_PATH` є такою ж змінною CMake:
