...
`include/ImagesAnnotatorDataImporters-0.11.0-dev/` поруч з
`include/ImagesAnnotatorDataImporters-0.11/`.
...
несе у собі пару "мажорна і мінорна версія" (`ImagesAnnotatorDataImporters011`),
...
|---|---|---|
| (немає) | `ImagesAnnotatorDataImporters-0.11` | `libImagesAnnotatorDataImporters-0.11.so` |
| `-DLIB_INCLUDE_MINOR_IN_NAME=OFF` | `ImagesAnnotatorDataImporters-0` | `libImagesAnnotatorDataImporters-0.so` |
| `-DLIB_INCLUDE_MICRO_IN_NAME=ON` | `ImagesAnnotatorDataImporters-0.11.0` | `libImagesAnnotatorDataImporters-0.11.0.so` |
| `-DLIB_NAME_SUFFIX=-dev` | `ImagesAnnotatorDataImporters-0.11-dev` | `libImagesAnnotatorDataImporters-0.11-dev.so` |
| `-DLIB_INCLUDE_MICRO_IN_NAME=ON -DLIB_NAME_SUFFIX=-dev` | `ImagesAnnotatorDataImporters-0.11.0-dev` | `libImagesAnnotatorDataImporters-0.11.0-dev.so` |
...
- `<prefix>/lib/libImagesAnnotatorDataImporters-0.11.so.0.11.0` разом із
  символьним посиланням soname `libImagesAnnotatorDataImporters-0.11.so.0`
  (`SOVERSION` залишається мажорною версією) і `.so`-посиланням для розробки,
- `<prefix>/include/ImagesAnnotatorDataImporters-0.11/*.h` - публічні заголовки з
  [src/lib/facade/public](/src/lib/facade/public), оголошені через
  `INSTALL_INTERFACE` цілі бібліотеки,
- `<libdir>/cmake/ImagesAnnotatorDataImporters-0.11/` зі згенерованими файлами
  `Config.cmake`, `ConfigVersion.cmake` і `Targets.cmake`, а також імпортовану
  ціль `ImagesAnnotatorDataImporters-0.11::ImagesAnnotatorDataImporters-0.11`.

Тому проект-споживач пише `find_package(ImagesAnnotatorDataImporters-0.11 0.11
...
`ImagesAnnotatorDataImporters011`, цими опціями не змінюється.
