...
`include/ImagesAnnotatorDataImporters-0.12.0-dev/` поруч з
...
несе у собі пару "мажорна і мінорна версія" (`ImagesAnnotatorDataImporters012`),
...
|---|---|---|
| (немає) | `ImagesAnnotatorDataImporters-0.12` | `libImagesAnnotatorDataImporters-0.12.so` |
...
- `<prefix>/lib/libImagesAnnotatorDataImporters-0.12.so.0.12.0` разом із
  символьним посиланням soname `libImagesAnnotatorDataImporters-0.12.so.0`
  (`SOVERSION` залишається мажорною версією) і `.so`-посиланням для розробки,
- `<prefix>/include/ImagesAnnotatorDataImporters-0.12/*.h` - публічні заголовки з
  [src/lib/facade/public](/src/lib/facade/public), оголошені через
  `INSTALL_INTERFACE` цілі бібліотеки,
- `<libdir>/cmake/ImagesAnnotatorDataImporters-0.12/` зі згенерованими файлами
  `Config.cmake`, `ConfigVersion.cmake` і `Targets.cmake`, а також імпортовану
  ціль `ImagesAnnotatorDataImporters-0.12::ImagesAnnotatorDataImporters-0.12`.

Тому проект-споживач пише `find_package(ImagesAnnotatorDataImporters-0.12 0.12
...
`ImagesAnnotatorDataImporters012`, цими опціями не змінюється.
