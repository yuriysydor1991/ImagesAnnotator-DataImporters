...
Репозиторій будує єдину спільну бібліотеку - `libImagesAnnotatorDataImporters-0.14.so`, - яка перетворює набір даних для навчання на диску на базу даних анотацій ImagesAnnotator. Виконуваного файлу немає: окрім самої бібліотеки збірка створює лише тестові бінарники.
...
| Шлях | Вміст |
|---|---|
| [src/lib/facade/public](/src/lib/facade/public) | встановлювані публічні заголовки - увесь інтерфейс `ImagesAnnotatorDataImporters014` |
| [src/lib/facade/IADataImportersFacade.cpp](/src/lib/facade/IADataImportersFacade.cpp) | реалізація фасаду, точка входу бібліотеки |
...
Реалізація живе у просторах імен `iadi0impl`, `iannotator::importers`, `iannotator::importers::helpers`, `iannotator::importers::parsers` та `iannotator::importers::sizers`. Жоден із них не встановлюється - проект-споживач бачить лише `ImagesAnnotatorDataImporters014` (рекомендовано аліас `iadi`).
