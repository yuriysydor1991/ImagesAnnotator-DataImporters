...
| [src/importers](/src/importers) | по одній піддиректорії на розкладку набору даних - `PlainTxt`, `Yolo4` та `PyTorch`, - кожна несе власну реалізацію `IImporter`, свій `CMakeLists.txt` і свої модульні тести, плюс спільні для всіх `ImportersAliases.h` та базу `Folder2DBImporter` |
| [src/sizers](/src/sizers) | `create_builtin_image_sizer()` і необовʼязковий OpenCV-`IImageSizeFacility` за ним |
| [src/helpers](/src/helpers) | `IHelper.h` та `TypeHelper.h`, обидва складаються лише із заголовків |
...
Реалізація живе у просторах імен `iadi0impl`, `iannotator::importers`, `iannotator::importers::helpers` та `iannotator::importers::sizers`. Жоден із них не встановлюється - проект-споживач бачить лише `ImagesAnnotatorDataImporters011` (рекомендовано аліас `iadi`).
