...
- for example `include/ImagesAnnotatorDataImporters-0.11.0-dev/` next to
...
carries the major and the minor version pair (`ImagesAnnotatorDataImporters011`)
...
|---|---|---|
| (none) | `ImagesAnnotatorDataImporters-0.11` | `libImagesAnnotatorDataImporters-0.11.so` |
...
- `<prefix>/lib/libImagesAnnotatorDataImporters-0.11.so.0.11.0` with the
  `libImagesAnnotatorDataImporters-0.11.so.0` soname symlink (the `SOVERSION`
  stays the major version) and the development `.so` namelink,
- `<prefix>/include/ImagesAnnotatorDataImporters-0.11/*.h` - the public headers of
  [src/lib/facade/public](/src/lib/facade/public), declared through the
  `INSTALL_INTERFACE` of the library target,
- `<libdir>/cmake/ImagesAnnotatorDataImporters-0.11/` with the generated
...
A consumer therefore writes `find_package(ImagesAnnotatorDataImporters-0.11 0.11
...
`ImagesAnnotatorDataImporters011`, is not affected by these options.
