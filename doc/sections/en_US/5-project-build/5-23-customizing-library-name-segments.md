...
- for example `include/ImagesAnnotatorDataImporters-0.14.0-dev/` next to
...
carries the major and the minor version pair (`ImagesAnnotatorDataImporters014`)
...
Resulting names for this `0.14.0` project:

| Configure flags | Library name | Produced binary |
|---|---|---|
| (none) | `ImagesAnnotatorDataImporters-0.14` | `libImagesAnnotatorDataImporters-0.14.so` |
...
- `<prefix>/lib/libImagesAnnotatorDataImporters-0.14.so.0.14.0` with the
  `libImagesAnnotatorDataImporters-0.14.so.0` soname symlink (the `SOVERSION`
  stays the major version) and the development `.so` namelink,
- `<prefix>/include/ImagesAnnotatorDataImporters-0.14/*.h` - the public headers of
  [src/lib/facade/public](/src/lib/facade/public), declared through the
  `INSTALL_INTERFACE` of the library target,
- `<libdir>/cmake/ImagesAnnotatorDataImporters-0.14/` with the generated
...
A consumer therefore writes `find_package(ImagesAnnotatorDataImporters-0.14 0.14
...
`ImagesAnnotatorDataImporters014`, is not affected by these options.
