...
The package file should be generated inside the project build root directory. For example, if project name wasn't changed and it's version is `0.11.0` the package name may look like `ImagesAnnotatorDataImporters-0.11.0-amd64.pkg` (the actual architecture suffix depends on the build host).
...
- `TEMPLATE_PROJECT_FREEBSD_PKG_ORIGIN` - FreeBSD ports origin in `<category>/<name>` form. The name part is the installable library name lowercased, so with the default settings the origin is `misc/imagesannotatordataimporters-0.11`.
