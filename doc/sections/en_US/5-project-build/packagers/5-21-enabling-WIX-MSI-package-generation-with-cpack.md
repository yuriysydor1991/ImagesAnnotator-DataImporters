...
The package file should be generated inside the project build root directory. For example, if the project name wasn't changed and its version is `0.14.0` the package name may look like `ImagesAnnotatorDataImporters-0.14.0-win64.msi` (the exact architecture suffix depends on the build host).
