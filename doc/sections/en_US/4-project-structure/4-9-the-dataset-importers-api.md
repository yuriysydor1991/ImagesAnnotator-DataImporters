...
The whole interface of the library is declared by the header files under [src/lib/facade/public](/src/lib/facade/public) and gets installed into the `include/ImagesAnnotatorDataImporters-0.11` sub-directory of the chosen install prefix. Nothing else leaves the shared object: every implementation class stays behind the abstract interfaces described here.

All the installable declarations live in the `ImagesAnnotatorDataImporters011` namespace. The name carries the library major and minor version numbers (`0.11` gives the `011` suffix) so that two library versions may coexist in a single translation unit. Alias it once in your own code:
