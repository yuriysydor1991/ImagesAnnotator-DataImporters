...
The three importer suites also cover the shared [src/importers/Folder2DBImporter.cpp](/src/importers/Folder2DBImporter.cpp) base, since every one of them compiles and drives it. Each of them runs a stand-in image size reader of its own - one reporting a fixed size and one failing every call - so what an import makes of either is covered whether or not this build has OpenCV. Every case works over a directory tree the fixture builds under the Google Test temporary directory and removes again, so the suite touches nothing else and needs no test data of its own.
...
With both options on the suite counts 90 test cases:
